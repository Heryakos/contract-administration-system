...
import type { Contract, UpdateContract, Vendor, ContractType, ContractCategory } from '../../../models/contract.model';
...
export class ContractGeneratorComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Detect edit mode by presence of id param
    ...
    if (idFromParams) {
      ...
      // Reset to initial state for new contracts
  ...
  private resetContractState(): void {
    ...
    this.contractGeneratorService.updateBlocks([
      { id: 'default-heading', type: 'heading', label: 'Service Agreement' }
    ]);
    
    this.contractGeneratorService.updateMetadata({
      templateId: '',
      partyA: '',
      partyB: '',
      effectiveDate: '',
  ...
  saveDraft(): void {
    ...
    if (this.isEditMode && this.currentContractId) {
      const payload: UpdateContract = {
        ...
        riskLevel: this.metadata.riskLevel || 'Low',
        status: 'Draft',
        contractTypeID: this.metadata.contractTypeID,
        categoryID: this.metadata.categoryID,
        vendorID: this.metadata.vendorID,
        version: this.metadata.version || '1',
        modifiedByUserID: employee?.user_ID || employee?.employee_Id,
        createdByUserID: this.loadedContract?.createdByUserID // Preserve the original creator
      };
      ...
      // For new contracts, use the service method that sets status to Draft
      this.contractGeneratorService.saveContract('Draft')
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: () => {
            this.isSaving = false;
  ...
  submitForApproval(): void {
    ...
    if (this.isEditMode && this.currentContractId) {
      ...
      // For new contracts, use the service method that sets status to Under Review
      this.contractGeneratorService.saveContract('Under Review')
        .pipe(takeUntil(this.destroy$))
        .subscribe({
          next: () => {
            this.isSaving = false;
  ...
  private calculateEndDate(): Date | undefined {
    if (this.metadata.effectiveDate && this.metadata.duration) {
      const startDate = new Date(this.metadata.effectiveDate);
      startDate.setMonth(startDate.getMonth() + this.metadata.duration);
      return startDate;
    }
    return undefined;
  }
  ...
  getBlockIcon(type: ContractBlock['type']): string {
    const blockType = this.blockTypes.find(bt => bt.type === type);
    return blockType ? blockType.icon : 'block';
  }

  getBlockLabel(type: ContractBlock['type']): string {
    const blockType = this.blockTypes.find(bt => bt.type === type);
  ...
  onReset(): void {
    this.resetContractState();
    this.snackBar.open('Contract reset successfully!', 'Close', { duration: 3000 });
  }
